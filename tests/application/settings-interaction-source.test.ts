...
describe("settings interaction source hygiene", () => {
  it("uses structured inline errors for team-member creation", () => {
    ...
    expect(actionSource).toMatch(/export async function createTeamMemberAction/);
    expect(componentSource).toMatch(/createTeamMemberAction/);
    expect(componentSource).not.toMatch(/action=\{createTeamMember\}/);
    expect(componentSource).toMatch(/memberCreateError/);
  });
  ...
  it("uses structured inline errors for team-member deactivation", () => {
    ...
    expect(componentSource).toMatch(/memberActionError/);
  });
