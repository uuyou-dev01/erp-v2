...
export default async function MySupplyOfferDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">供给方：{offer.ownerPartner?.name ?? "本店自有"} · 发货地：{offer.shipFromLocation ?? "-"}</p>
        </div>
        <div className="flex flex-col gap-2 sm:items-end">
          <SupplyOfferActions id={offer.id} status={offer.status} title={offer.title} />
          <Link href="/marketplace/my-offers">
            <Button variant="ghost" size="sm">返回我的供给</Button>
          </Link>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">明细数</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-bold">{offer.items.length}</CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          {offer.visibility === "PARTNER_ONLY" ? (
            offer.visibilityRules.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground">当前选择了合作方可见，但还没有指定可见对象。</p>
            )
          ) : (
          ...
          <div className="divide-y divide-border">
            {offer.items.map((item: SerializedSupplyOfferItem) => (
              <div key={item.id} className="grid gap-3 py-4 md:grid-cols-[1fr_auto_auto] md:items-center">
                ...
                <div className="text-sm">数量 <span className="font-medium">{item.quantityAvailable}</span></div>
                <div className="text-sm">单价 <span className="font-medium">{formatMoney(item.currency, item.unitPrice)}</span></div>
              </div>
            ))}
          </div>
        </CardContent>
