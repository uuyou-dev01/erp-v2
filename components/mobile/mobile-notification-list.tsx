    ...
    <>
      <div className="flex justify-end py-3">
        <Button
          ...
          onClick={async () => {
            await fetch("/api/v1/mobile/notifications/read-all", { method: "POST" });
            router.refresh();
          }}
        >
      ...
      <div className="divide-y divide-slate-100">
        {notifications.map((item) => (
          <Link
            ...
            className="grid grid-cols-[36px_1fr] gap-3 py-4"
          >
            <span
              className={`flex h-9 w-9 items-center justify-center rounded-full ${item.readAt ? "bg-slate-100 text-slate-400" : "bg-blue-50 text-blue-600"}`}
            >
