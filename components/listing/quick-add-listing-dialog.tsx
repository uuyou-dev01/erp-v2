...
export function QuickAddListingDialog({
  ...
  const [currency, setCurrency] = useState(product.referenceCurrency ?? "CNY");
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getPlatforms(storeId).then((rows) => {
      if (cancelled) return;
              ...
              >
                {sellableUnits.map((u) => (
                  <option key={u.id} value={u.id}>
                    品相 {formatItemUnitCondition(u.conditionGrade)} · {u.locationName}
                  </option>
                ))}
              </select>
            </div>
