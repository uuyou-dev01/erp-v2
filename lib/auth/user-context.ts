...
export async function requireUserContext(input?: {
  ...
  const activeStoreId =
    input?.storeId ??
    (cookieStoreId && storeIds.includes(cookieStoreId)
      ? cookieStoreId
      : undefined) ??
    storeIds[0] ??
    user.storeId;
  ...
  const salesChannelAccountIds = user.channelAccesses.map(
    (access) => access.salesChannelAccountId,
  );
  ...
  const activePool = await prisma.inventoryPool.findFirst({
    where: {
      ...
      OR: [
        { id: { in: inventoryPoolIds } },
        { legacyStoreId: activeStoreId },
      ],
    },
    select: { id: true },
  });
