...
const MOBILE_ASSET_ROOT = path.resolve(process.cwd(), ".data", "mobile-assets");

export function resolveMobileAssetPath(storageKey: string) {
  const absolutePath = path.resolve(MOBILE_ASSET_ROOT, storageKey);
  if (!absolutePath.startsWith(`${MOBILE_ASSET_ROOT}${path.sep}`)) {
    throw new Error("证据文件路径不安全");
  }
  return absolutePath;
}
...
export async function readMobileAsset(storageKey: string) {
  const config = s3Config();
  if (!config) return readFile(resolveMobileAssetPath(storageKey));
