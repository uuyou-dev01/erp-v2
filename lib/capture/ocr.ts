...
export async function recognizeCaptureImage(image: Buffer, language: "chi_sim" | "jpn" | "eng" = "chi_sim") {
      ...
      try {
        ...
        for (const line of lines) {
          for (const match of line.matchAll(pricePattern)) {
            ...
            priceCandidates.push({ value, confidence: Math.max(0, Math.min(1, result.data.confidence / 100)), evidence: line });
          }
        }
        const trackingCandidates = [...new Set(lines.flatMap((line) => line.match(/\b[A-Z0-9-]{10,30}\b/gi) ?? []))].slice(0, 8);
        return {
          text,
