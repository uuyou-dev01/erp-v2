...
3) 商品组 / 规格 SKU / 独立 SKU 与页面统计口径

商品组是本系统的 SPU-like 款型容器，例如 iPhone 16 Pro 系列、AJ1 芝加哥 2015、盲盒系列、火影忍者晓组织系列或服装款式。规格 SKU 表示可交易规格，例如具体颜色/容量、尺码、角色、长度或颜色尺码组合。独立 SKU 表示没有规格拆分、商品本身即可直接交易的商品。

库存对象必须挂在规格 SKU 或独立 SKU 下，不能挂在商品组下：

- InventoryLot：同质新品数量库存。
- ItemUnit：具体物理单件，适合中古、瑕疵、唯一件、寄售件、需要照片、需要贴标或合规追踪的库存。
- 商品组：只聚合规格、图片、品牌货号/款号、说明和统计，不直接采购、入库、上架或销售。

页面统计规则：

- 商品组页：汇总所有规格 SKU 的 Lot 数量、ItemUnit 件数、上架、采购、销售与利润参考。
- 规格 SKU 页：只展示自身 Lot 与 ItemUnit。
- 独立 SKU 页：按自身展示 Lot 与 ItemUnit，不出现规格切换。
- 库存看板：商品组卡片展示规格 SKU 汇总；新品批次和单件库存分区展示。
- 商品档案 / 库存批次：默认优先按商品组展示；若历史数据还没有商品组，但同品牌/分类/系列或中文命名前缀能识别为同一系列，可作为“展示分组”临时聚合。展示分组只改善查找与扫描，不写回 SKU.parentSkuId，也不影响 StockLedger、OrderLine 或 Listing 的统计真相。
- 单件库存页：是 ItemUnit 工作台，用于核对 SKU 层级、标签、图片、库位、状态和上架，不是第二套商品主档。

表单与列表规则：

- 新增商品组时，表单只要求商品组名称；品牌、品类、品牌货号/款号、规格维度是高优先级可选项。
- 新增规格 SKU 时，用户只填写短规格值，例如 `42码`、`小南`、`10cm`、`M`，系统生成完整展示名和内部 SKU 编码。
- 商品主档列表默认先展示商品组，再以 chip、tab、展开行或子行展示规格 SKU。
- 采购、入库、上架、销售等业务选择器默认只允许选择规格 SKU 或独立 SKU；搜索到商品组时，应引导选择其下具体规格。
